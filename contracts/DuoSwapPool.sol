...
contract DuoSwapPool is IDuoSwapPool, Context {
    ...
    event PoolInitialized(address indexed who, uint256 amountA, uint256 amountB, uint256 liquidityMinted);
    event Withdrawn(address indexed who, uint256 amountA, uint256 amountB, uint256 liquidityWithdrawn);
    event Deposited(address indexed who, uint256 amountA, uint256 amountB, uint256 liquidityMinted);
        ...
        returns (uint256)
    {
        ...
        require(
            totalLiqudity == 0,
            "DuoSwap: pool already initialized"
        );
        ...
        liquidityOf[_msgSender()] = liquidityOf[_msgSender()].add(liquidityShares);
        ...
        returns (uint256, uint256)
    {
        ...
        uint percentShareOfLiquidityToWithdraw = liquidityShares.mul(100).div(totalLiqudity);
        
        uint amountA = aReserves.mul(100).div(percentShareOfLiquidityToWithdraw);
        uint amountB = bReserves.mul(100).div(percentShareOfLiquidityToWithdraw);
        
        liquidityOf[_msgSender()] = liquidityOf[_msgSender()].sub(liquidityShares);
    ...
    function deposit(uint256 amountA, uint256 amountB)
        ...
        returns (uint256)
    {
        uint256 liquidityShares = _getLiquiditySharesMinted(amountA, amountB, aReserves, bReserves);
    ...
    function _swap(uint256 amount, ERC20 inputToken, ERC20 outputToken)
        private
        returns (uint256)
    {
        ...
        uint256 outputTokenAmount = _getOutputAmount(amount, inputReserves, outputReserves);
        ...
        require(outputToken.transferFrom(address(this), _msgSender(), outputTokenAmount));
        
        return outputTokenAmount;
    }
    
    
    function _getOutputAmount(uint256 inputAmount, uint256 inputReserves, uint256 outputReserves)
        private
        pure
        returns (uint256)
    {
